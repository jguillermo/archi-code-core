...
import { canBeJson, canBeJsonObject } from '../../src/canBe';
...
describe('canBeJsonObject', () => {
  it('is an alias of canBeJson', () => {
    expect(canBeJsonObject).toBe(canBeJson);
    expect(canBeJsonObject('{"a":1}')).toBe(true);
    expect(canBeJsonObject('[1]')).toBe(false);
  });
});
