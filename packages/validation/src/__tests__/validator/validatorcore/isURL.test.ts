...
describe('Validators', () => {
  ...
  it('should validate URLs without protocol', () => {
    ...
    test({
      ...
      valid: ['http://localhost:3000', 'https://service-name:8080', 'custom://localhost'],
      invalid: ['localhost:3000', 'service-name:8080', 'user:password@localhost'],
    });
  ...
  it('should reject authentication strings if a protocol is required', () => {
    test({
      ...
      invalid: ['user:pw@foobar.com/', 'user:password@example.com', 'admin:pass@ftp.example.com/'],
    });
  });
  ...
  it('GHSA-9965-vmph-33xx vulnerability - protocol delimiter parsing difference', () => {
    ...
    test({
      ...
      valid: [],
      invalid: ["javascript:alert(1);a=';@example.com/alert(1)"],
    });
  });
