...
const CH = (str: string): boolean => {
  // @see {@link https://www.ech.ch/de/ech/ech-0097/5.2.0}
  const hasValidCheckNumber = (digits: number[]): boolean => {
...
export const vatMatchers = {
  ...
  PL: (str: string): boolean =>
    /^(PL)?(\d{10}|(\d{3}-\d{3}-\d{2}-\d{2})|(\d{3}-\d{2}-\d{2}-\d{3}))$/.test(str),
  PT,
...
export default function isVAT(input: unknown, countryCode: VATCountryCode): boolean {
