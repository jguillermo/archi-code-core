...
describe('UserEmailPasswordRegisterDto', () => {
  it('all correct data', async () => {
    const object = Builder(UserEmailPasswordRegisterDto).name('testUser').email('123456@mail.com').password('admin').build();
    const errors = await validate(object);
    expect(errors.length).toEqual(0);
  });

  it('send password  empty error', async () => {
    const object = Builder(UserEmailPasswordRegisterDto).name('testUser').email('123456@mail.com').build();
    const errors = await validate(object);
  ...
  it('send email empty error', async () => {
    const object = Builder(UserEmailPasswordRegisterDto).name('testUser').password('admin').build();
    const errors = await validate(object);
    expect(errors.length).toEqual(1);
  ...
  it('send empty data', async () => {
    const object = Builder(UserEmailPasswordRegisterDto).build();
    const errors = await validate(object);
